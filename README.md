...
- Генерация full-tunnel клиентских конфигов для IPv4 и IPv6: `AllowedIPs = 0.0.0.0/0, ::/0`.
- Лимиты трафика для клиентов: без лимита, GB/MB-лимит или произвольное значение.
...
5. При необходимости задайте лимит трафика, сбросьте счётчик или разблокируйте клиента после увеличения лимита.
6. При необходимости заблокируйте, разблокируйте или удалите клиента из списка.
7. После ручных изменений WireGuard-конфига используйте кнопку перезапуска интерфейса.

## Лимиты трафика

Для каждого клиента можно оставить режим "без лимита" или задать объём, например `10 GB`, `100 GB`, `2048 MB`. Лимит можно указать при создании клиента или изменить позже кнопкой "Лимит" в карточке клиента.

Панель считает трафик как сумму входящего и исходящего трафика peer:

```text
traffic_used_bytes = received_bytes + transmitted_bytes
```

Данные берутся из WireGuard runtime-статистики через `wg show <interface> dump`. WireGuard может сбрасывать runtime-счётчики после перезапуска интерфейса, поэтому панель хранит накопленный счётчик в `data/peers.json` и добавляет только разницу с последнего наблюдения. Если runtime-счётчик стал меньше предыдущего значения, панель считает это сбросом WireGuard и продолжает накопление с нового значения.

Фоновая проверка лимитов запускается внутри Node.js-процесса примерно раз в 30 секунд и использует ту же serial queue, что операции добавления, блокировки и удаления клиентов.

Когда включённый лимит достигнут:

- клиент помечается как `traffic_blocked`;
- клиент остаётся в панели и не удаляется;
- доступ к VPN отключается существующим механизмом блокировки peer через `AllowedIPs = 0.0.0.0/32`;
- в карточке появляется статус "лимит исчерпан".

Чтобы вернуть доступ, администратор должен сделать одно из действий:

- нажать "Сбросить трафик" — счётчик станет `0`, блокировка по лимиту снимется;
- увеличить лимит выше текущего использования и нажать "Разблокировать лимит";
- выключить лимит для клиента.

Ручная блокировка и блокировка по лимиту независимы. Если клиент был заблокирован вручную, сброс трафика не включит ему доступ, пока администратор не нажмёт обычную кнопку разблокировки.
...
```json
{
  ...
  "traffic_updated_at": 1710000000000,
  "traffic_last_runtime_bytes": 123456789
}
```
...
| `traffic_limit_enabled` | Включён ли лимит трафика. |
| `traffic_limit_bytes` | Лимит трафика в байтах. |
| `traffic_used_bytes` | Накопленный использованный трафик в байтах. |
| `traffic_blocked` | Заблокирован ли клиент из-за исчерпания лимита. |
| `traffic_reset_at` | Время последнего сброса счётчика или `null`. |
| `traffic_updated_at` | Время последнего обновления traffic-статистики. |
| `traffic_last_runtime_bytes` | Внутреннее последнее значение WireGuard runtime-счётчика для корректного накопления после рестартов. |
...
- Клиентские `.conf` направляют IPv4 и IPv6 через VPN, но kill switch нужно включать отдельно на устройстве пользователя.
