...
## Разрешение конфликтов PR

Если GitHub показывает конфликт в `README.md` и `scripts/preinstall-wg.sh`, оставляйте версию, где:
- есть команда `cp .env.example .env`;
- `preinstall-wg.sh` **не** делает безусловный `apt install npm` вместе с NodeSource `nodejs`;
- preinstall сначала проверяет наличие `node`/`npm`, затем ставит только недостающее.
